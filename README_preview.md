...
[![Open in Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/USERNAME/REPO/blob/main/notebooks/gstride_psychointelligence.ipynb)
...
![Algorithm comparison](figures/09_algorithm_comparison.png)

---

## Reproducing this

The dataset is included in the repository, so this should work in one go.
...
That prints every headline number in this README in about two minutes. Everything is seeded with `RANDOM_STATE = 42`, so your output should match ours exactly. If it doesn't, that's a bug and we'd like to hear about it.

For the full analysis with all the charts and explanation, open the notebook:

```bash
jupyter notebook notebooks/gstride_psychointelligence.ipynb
```

The notebook is committed **with its outputs intact**, so you can read the whole thing on GitHub without running anything.
...
.
├── notebooks/
│   └── gstride_psychointelligence.ipynb   Full analysis, 14 steps, outputs included
├── src/
│   └── reproduce.py                       Headline numbers without Jupyter
├── data/
│   ├── Database_register.xlsx             GSTRIDE register (163 participants)
│   ├── Database_register.csv              Same data, semicolon-delimited
│   └── README.md                          Provenance, licence, column guide
├── figures/                               All 12 charts as PNG
├── docs/
│   └── methodology.md                     Design decisions and why we made them
├── requirements.txt
├── CITATION.cff
└── LICENSE
```

If you only read one thing, read the notebook. It's written for someone who has never trained a model, and every step explains why it's there, not just what it does.
...
![Target balance](figures/02_target_balance.png)
...
Before modelling anything, it's worth checking there's a signal to find at all. There is, and you can see it by eye:

![Signal boxplots](figures/07_signal_boxplots.png)

People with signs of cognitive decline take noticeably longer on the Timed Up and Go test and walk more slowly. That's the effect the model goes on to formalise.

See [`data/README.md`](data/README.md) for the full column guide and attribution.
...
Three fixes, one of which mattered more than it looks:

Eight rows record the fall outcome as `"NO "` with a trailing space. Python treats that as a different value from `"NO"`, so without stripping whitespace those eight people fail to map to either class and quietly disappear. You'd analyse 155 people while believing you had 163, and nothing would warn you.

Age arrives as bands (`"70-74"`), so we take the midpoint. And we binarise the three outcomes at clinically conventional thresholds.
...
You can see the entanglement in the correlation structure before you even quantify it:

![Correlation heatmap](figures/04_correlation_heatmap.png)

Variance inflation factor made the scale of the problem obvious:

![VIF before cleaning](figures/05_vif_before_cleaning.png)
...
With 163 people, a single train/test split is a coin toss. We use 5-fold cross-validation repeated 10 times, so every participant is held out and tested on, and the reported figure is an average of 50 separate fits.

![Cross-validation scheme](figures/08_cross_validation_scheme.png)

We also run all three outcomes through the same pipeline with the same six features. That's what makes the separability test in the results possible.
...
![ROC and confusion matrix](figures/10_roc_and_confusion_matrix.png)
...
![Odds ratios](figures/11_odds_ratios.png)
...
![Task alignment](figures/12_task_alignment.png)
...
On the psycho-affective layer: GSTRIDE includes a fear-of-falling questionnaire (FES-I), and we intended to use it. Eighteen participants are marked "Incapable" rather than given a score, and that missingness is not random — mean GDS is 1.70 among those who completed it and 4.83 among those who couldn't. The people who couldn't self-report are the ones with advanced dementia.
...
For this analysis specifically, see [`CITATION.cff`](CITATION.cff).
...
The GSTRIDE data redistributed in `data/` is **CC BY 4.0**, which permits reuse and redistribution provided the original authors are credited. That attribution is not optional — details in [`data/README.md`](data/README.md).
